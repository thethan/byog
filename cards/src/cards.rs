...
impl CsvIngestionError {
    fn new(row: usize, message: impl Into<String>) -> Self {
        Self {
            row,
...
pub fn parse_csv(input: &str) -> Result<Vec<Vec<String>>, CsvIngestionError> {
    ...
    if quoted {
        return Err(CsvIngestionError::new(
            rows.len() + 1,
...
fn records(
    input: &str,
    required: &[&str],
) -> Result<Vec<(usize, HashMap<String, String>)>, CsvIngestionError> {
...
fn required_value<'a>(
    ...
    name: &str,
    row: usize,
) -> Result<&'a str, CsvIngestionError> {
    values
        .get(name)
        .map(String::as_str)
        .map(str::trim)
...
fn checked_id(value: &str, field: &str, row: usize) -> Result<String, CsvIngestionError> {
    ...
    valid_id(&id)
        .then_some(id)
        .ok_or_else(|| CsvIngestionError::new(row, format!("'{field}' is not a valid identifier")))
}
...
pub fn parse_card_cost(value: &str, row: usize) -> Result<Option<CardCost>, CsvIngestionError> {
    ...
    for part in value.split('|') {
        ...
        let (resource, amount) = match pieces.as_slice() {
            [amount] => ("generic", *amount),
            [resource, amount] => (*resource, *amount),
            _ => {
                return Err(CsvIngestionError::new(
                    row,
                    format!("invalid cost component '{token}'"),
                ));
            }
        };
        let resource = normalize_id(resource);
        let amount = amount.parse::<u32>().ok().filter(|v| *v > 0);
        if !valid_id(&resource) || amount.is_none() {
            return Err(CsvIngestionError::new(
                row,
                format!("invalid cost component '{token}'"),
            ));
        }
        components.push(CardCostComponent {
            resource,
    ...
    records(input, &["game_id", "id", "name"])?
        ...
        .map(|(row, values)| {
            ...
            if !seen.insert(format!("{game_id}/{id}")) {
                return Err(CsvIngestionError::new(
                    row,
                    format!("duplicate card type '{game_id}/{id}'"),
                ));
            }
            Ok(CardType {
                game_id,
                id,
                name: required_value(&values, "name", row)?.to_owned(),
...
pub fn ingest_cards_csv(
    input: &str,
    card_types: &[CardType],
) -> Result<Vec<Card>, CsvIngestionError> {
    let type_keys: HashSet<String> = card_types
        .iter()
        .map(|t| format!("{}/{}", t.game_id, t.id))
        .collect();
    let mut seen = HashSet::new();
    records(input, &["game_id", "id", "name", "card_type_id"])?
        .into_iter()
        .map(|(row, values)| {
            let game_id = checked_id(required_value(&values, "game_id", row)?, "game_id", row)?;
            let id = checked_id(required_value(&values, "id", row)?, "id", row)?;
            let card_type_id = checked_id(
                required_value(&values, "card_type_id", row)?,
                "card_type_id",
                row,
            )?;
            if !seen.insert(format!("{game_id}/{id}")) {
                return Err(CsvIngestionError::new(
                    row,
                    format!("duplicate card '{game_id}/{id}'"),
                ));
            }
            if !type_keys.contains(&format!("{game_id}/{card_type_id}")) {
                return Err(CsvIngestionError::new(
                    row,
                    format!("unknown card type '{game_id}/{card_type_id}'"),
                ));
            }
...
pub fn resolve_card_visual(card: &Card, card_type: &CardType) -> Result<CardVisual, String> {
    if card.game_id != card_type.game_id || card.card_type_id != card_type.id {
        return Err(format!(
            "card type '{}/{}' does not belong to card '{}/{}'",
            card_type.game_id, card_type.id, card.game_id, card.id
        ));
    }
    Ok(match &card.visual {
        CardVisual::FullImage { image } => CardVisual::FullImage {
            image: image.clone(),
        },
        CardVisual::Generated {
            ...
            background_image: background_image
                .clone()
                .or_else(|| card_type.background_image.clone()),
            background_color: background_color
                .clone()
                .or_else(|| card_type.background_color.clone()),
            icon: icon.clone().or_else(|| card_type.icon.clone()),
        },
    })
}

pub fn resolve_card_appearance(
    card: &Card,
    card_type: &CardType,
) -> Result<Option<CardAppearance>, String> {
    match resolve_card_visual(card, card_type)? {
        ...
        CardVisual::Generated {
            ...
            back_logo: card
                .back_logo
                .clone()
                .or_else(|| card_type.back_logo.clone()),
        })),
    }
}
...
mod tests {
    ...
    fn ingests_cost_and_inherits_appearance() {
        ...
        assert_eq!(
            cards[0].cost.as_ref().unwrap().components[1],
            CardCostComponent {
                resource: "fire".into(),
                amount: 2
            }
        );
        let appearance = resolve_card_appearance(&cards[0], &types[0])
            .unwrap()
            .unwrap();
    ...
    fn full_image_suppresses_generated_appearance() {
        ...
        assert!(matches!(
            resolve_card_visual(&cards[0], &types[0]).unwrap(),
            CardVisual::FullImage { .. }
        ));
    ...
    fn supports_quoted_commas() {
        let types = ingest_card_types_csv(
            "game_id,id,name,description\narcana,event,Event,\"Resolve once, then discard\"",
        )
        .unwrap();
        assert_eq!(
            types[0].description.as_deref(),
            Some("Resolve once, then discard")
        );
    }
}
